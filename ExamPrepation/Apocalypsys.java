...
public class Apocalypsys {
    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);
        ArrayDeque<Integer> textile = Arrays.stream(scanner.nextLine()
                        .split(" "))
...
//        });
        printMap.entrySet().stream().sorted((e1, e2) -> e2.getValue().compareTo(e1.getValue()))
                .sorted((e1, e2) -> {
                    int compareByAmountCreated = e2.getValue().compareTo(e1.getValue());
                    if (compareByAmountCreated != 0) { // If names are not the same return the value
                        return compareByAmountCreated;
                    }
                    return e1.getKey().compareTo(e2.getKey()); // else return comparison by name
                }).forEach(entry -> {
                            if (entry.getValue() > 0) {
                                System.out.println(entry.getKey() + " - " + entry.getValue());
                            }
        ...
        if (!textile.isEmpty()) {
            ...
            String collect = textile.stream().map(Object::toString).collect(Collectors.joining(", "));
        ...
        if (!medicaments.isEmpty()) {
            ...
            String collect = medicaments.stream().map(Object::toString).collect(Collectors.joining(", "));
            System.out.println(collect);
        }
